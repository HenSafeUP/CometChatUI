...
/**
 The `CometChatSnackBoard` class provides the interface for showing and hiding messages.
 ...
 */
open class CometChatSnackBoard {
    
    
    @objc public  enum CometChatSnackBoardDuration: Int {
        case short = 1
        case middle = 3
        case long = 5
    ...
    public enum PresentationContext {
        ...
        */
        ...
        /**
         ...
         CometChatSnackBoard automatically increases the top margins of any message
        ...
        */
        ...
        /**
         Displays the message in a new window, at the specified window level,
         in the specified window scene. CometChatSnackBoard automatically increases the top margins
    ...
    public enum DimMode {
        ...
        */
        ...
        /**
         ...
         CometChatSnackBoard does not apply alpha transparency to the color, so any alpha
    ...
    */
    ...
    public struct Config {
        ...
        */
        ...
        /**
         The user interface style to use when CometChatSnackBoard displays a message its own window.
        ...
        */
        ...
        /**
         If specified, CometChatSnackBoard calls this closure when an instance of
         `WindowViewController` is needed. Use this if you need to supply a custom subclass
         of `WindowViewController`.
         */
        public var windowViewController: ((_ config: CometChatSnackBoard.Config) -> WindowViewController)?
    ...
    fileprivate let messageQueue = DispatchQueue(label: "it.swiftkick.CometChatSnackBoard", attributes: [])
...
extension CometChatSnackBoard: PresenterDelegate {
...
/**
 ...
 CometChatSnackBoard provides several default nib files in the Resources folder that can be
...
extension CometChatSnackBoard {
    ...
    /**
     ...
     first followed by the CometChatSnackBoard bundle.
    ...
    /**
     ...
     with matching type `T`. The main bundle is searched first followed by the CometChatSnackBoard bundle.
    ...
    fileprivate class func internalViewFromNib<T: UIView>(named name: String, bundle: Bundle? = nil, filesOwner: AnyObject = NSNull.init()) throws -> T {
        ...
        #if swift(>=4.1)
        guard let view = arrayOfViews.compactMap( { $0 as? T} ).first else { throw CometChatSnackBoardError.cannotLoadViewFromNib(nibName: name) }
        #else
        guard let view = arrayOfViews.flatMap( { $0 as? T} ).first else { throw CometChatSnackBoardError.cannotLoadViewFromNib(nibName: name) }
        #endif
        return view
    }
}
...
/*
 ...
 This extension provides a shared instance of `CometChatSnackBoard` and a static API wrapper around
 this instance for simplified syntax. For example, `CometChatSnackBoard.show()` is equivalent
 to `CometChatSnackBoard.sharedInstance.show()`.
 */

extension CometChatSnackBoard {
    
    /**
     A default shared instance of `CometChatSnackBoard`. The `CometChatSnackBoard` class provides
     a set of static APIs that wrap calls to this instance. For example, `CometChatSnackBoard.show()`
     is equivalent to `CometChatSnackBoard.sharedInstance.show()`.
     */
    public static var sharedInstance: CometChatSnackBoard {
    ...
    public static func display(message: String, mode: Theme, duration: CometChatSnackBoardDuration) {
        let view = MessageView.viewFromNib(layout: .messageView)
        view.configureContent(title: message, body: "" , iconImage: nil, iconText: nil, buttonImage: nil, buttonTitle: "", buttonTapHandler: { _ in CometChatSnackBoard.hide() })
        view.configureTheme(mode, iconStyle: .default)
        var config = CometChatSnackBoard.defaultConfig
        config.duration = .seconds(seconds: TimeInterval(duration.rawValue))
        CometChatSnackBoard.show(config: config, view: view)
    }
    
    public static func show(message: String, mode: Theme, duration: CometChatSnackBoardDuration) {
        let view = MessageView.viewFromNib(layout: .messageView)
        view.configureContent(title: message, body: "" , iconImage: nil, iconText: nil, buttonImage: nil, buttonTitle: "", buttonTapHandler: { _ in CometChatSnackBoard.hide() })
        view.configureTheme(mode, iconStyle: .default)
        var config = CometChatSnackBoard.defaultConfig
